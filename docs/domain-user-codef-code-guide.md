...
작성일: 2026-06-18 / 최종 갱신: 2026-06-23 (user 패키지 5줄 이상 주석 전체 이전 — 1장 보강, 1.12/1.13절 신설 / 2장 CodefAuthClient·CodefHttpServiceConfig를 RestClient 전환 이후 코드에 맞게 갱신)
...
**CodefHttpServiceConfig** — OAuth 토큰 발급 전용 `RestClient` + `CodefOAuthExchange`를 등록합니다. 다른 `*HttpServiceConfig`와 동일한 패턴으로 connect/read timeout(5초)과 `defaultStatusHandler`를 적용해, 4xx/5xx 응답을 받는 즉시 `BusinessException(CODEF_TOKEN_ISSUE_FAILED)`로 변환합니다. `CodefOAuthExchange.issueToken()`의 `@PostExchange`가 `oauth.codef.io` 절대 URL을 직접 지정하므로 baseUrl은 설정하지 않습니다. 빌드 전 `RestClient.Builder`를 노출하는 `restClientBuilder()`를 따로 둔 이유는 `CodefOcrHttpServiceConfig`와 동일하게 `MockRestServiceServer` 테스트 바인딩 때문입니다(`CodefHttpServiceConfigTest`).
...
| 외부 연동 | Spring `RestClient`(EasyCodef SDK 우회), 전용 타임아웃 Bean 분리, OAuth 토큰 캐싱, `defaultStatusHandler` 기반 4xx/5xx 일괄 예외 변환 |
